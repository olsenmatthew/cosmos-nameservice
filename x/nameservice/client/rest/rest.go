...
import (
	...
	clientrest "github.com/olsenmatthew/cosmos-nameservice/client/rest"
...
func buyNameHandler(cdc *codec.Codec, cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		addr, err := sdkAccFromBech32(req.Buyer)
		if err != nil {
...
type setNameReq struct {
	BaseReq res.BaseReq `json:"base_req"`
	Name string `json:"name"`
...
func setNameHandler(cdc *codec.Codec, cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Reqests) {
		var req setNameReq
		if !rest.ReadRESTReq(w, r, cdc, &req) {
			rest.WriteErrorResponse(w, http.StatusBadReqest, "failed to parse request")
			return
		}
